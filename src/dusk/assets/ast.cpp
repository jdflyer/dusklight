...
bool tryRecoverAdpcm(
    ...
    for (int coefIdx = 0; coefIdx < 16; coefIdx++) {
        s16 coef1 = audio::Coefficient0[coefIdx];
        s16 coef2 = audio::Coefficient1[coefIdx];
        ...
        for (int scale = 0; scale <= 15; scale++) {
            ...
            for (int i = 0; i < samples.size() && ok; i++) {
                ...
                for (int n = -8; n <= 7; n++) {
                    s16 decoded = audio::Clamp16((n << scale) + prediction);
